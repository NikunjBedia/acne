...
def to_predict(result):
    ...
    url = 'https://acme123-frmwy.westus2.inference.ml.azure.com/score'
    # Replace this with the primary/secondary key or AMLToken for the endpoint
    api_key = '3ncSDIAuUEDMTHmvFHNFpH4Bag8tSj2r'
    if not api_key:
        raise Exception("A key should be provided to invoke the endpoint")

    # The azureml-model-deployment header will force the request to go to a specific deployment.
    # Remove this header to have the request observe the endpoint traffic rules
    headers = {'Content-Type':'application/json', 'Authorization':('Bearer '+ api_key), 'azureml-model-deployment': 'acnemodel-1' }
    df=pd.DataFrame()
    ...
    try:
        ...
        df = pd.DataFrame(data, columns=["Image_name", "Skin_type","Presence_of_Lesions","Redness_scale","Oiliness_scale","Texture_of_acne","Severity_level","Type_of_acne","Status"])
        print(df)
        # print(df.round(3))
